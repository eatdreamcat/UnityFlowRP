...
namespace UnityEditor.Rendering.SuperAdvanced
{
    ...
    {
        private SARenderGraphData.TextureBufferNode TextureBufferNode =>
        ...
        private VisualElement Header()
        {
            ...
            var isShadowMap = SAElementUtilities.CreateToggle(TextureBufferNode.isShadowMap, "Shadow Map", evt =>
            {
                TextureBufferNode.isShadowMap = evt.newValue;
            });
            root.Add(isShadowMap);
            
            var fallbackToBlack = SAElementUtilities.CreateToggle(TextureBufferNode.fallBackToBlackTexture, "Fall Back To Black", evt =>
            {
        ...
        private VisualElement Size()
        {
            ...
            var width = SAElementUtilities.CreateIntegerField(TextureBufferNode.width, "Width", evt =>
            {
                TextureBufferNode.width = evt.newValue;
            });
            
            var height = SAElementUtilities.CreateIntegerField(TextureBufferNode.height, "Height", evt =>
            {
                TextureBufferNode.height = evt.newValue;
            });

            var scale = SAElementUtilities.CreateVector2Field(TextureBufferNode.scale, "Scale", evt =>
            {
                TextureBufferNode.scale = evt.newValue;
            });
            
            var sizeMode = SAElementUtilities.CreateEnumField(TextureBufferNode.sizeMode, "Size Mode", evt =>
            {
            ...
            var useDynamicScale = SAElementUtilities.CreateToggle(TextureBufferNode.useDynamicScale,
                "Dynamic Scale",
                evt =>
                {
        ...
        private VisualElement InitState()
        {
            ...
            var clearBuffer = SAElementUtilities.CreateToggle(TextureBufferNode.clearBuffer, "Clear", evt =>
            {
                TextureBufferNode.clearBuffer = evt.newValue;
            });
            root.Add(clearBuffer);

            var colorField = SAElementUtilities.CreateField<ColorField, Color>(TextureBufferNode.clearColor,
                "Clear Color",
                evt =>
                {
        ...
        private VisualElement Format()
        {
            ...
            var graphicFormat = SAElementUtilities.CreateEnumField(TextureBufferNode.colorFormat, "Format", evt =>
            {
                TextureBufferNode.colorFormat = (GraphicsFormat)evt.newValue;
            });
            root.Add(graphicFormat);
            
            var depthBits = SAElementUtilities.CreateEnumField(TextureBufferNode.depthBits, "Depth Bits", evt =>
            {
                TextureBufferNode.depthBits = (DepthBits)evt.newValue;
            });
            root.Add(depthBits);

            var dimension = SAElementUtilities.CreateEnumField(TextureBufferNode.dimension, "Dimension", evt =>
            {
        ...
        private VisualElement FilterAddressing()
        {
            ...
            var filterMode = SAElementUtilities.CreateEnumField(TextureBufferNode.filterMode, "Filter Mode", evt =>
            {
                TextureBufferNode.filterMode = (FilterMode) evt.newValue;
            });
            root.Add(filterMode);

            var wrapMode = SAElementUtilities.CreateEnumField(TextureBufferNode.wrapMode, "Wrap Mode", evt =>
            {
                TextureBufferNode.wrapMode = (TextureWrapMode) evt.newValue;
            });
            root.Add(wrapMode);

            var anisoLevel = SAElementUtilities.CreateIntegerField(TextureBufferNode.anisoLevel, "Aniso Level", evt =>
            {
        ...
        private VisualElement Mipmap()
        {
            ...
            var useMipmap = SAElementUtilities.CreateToggle(TextureBufferNode.useMipMap, "Mipmap", evt =>
            {
                TextureBufferNode.useMipMap = evt.newValue;
            });
            root.Add(useMipmap);
            
            var autoGenerateMip = SAElementUtilities.CreateToggle(TextureBufferNode.autoGenerateMips, "Auto Generate Mipmap", evt =>
            {
                TextureBufferNode.autoGenerateMips = evt.newValue;
            });
            root.Add(autoGenerateMip);

            var mipMapBias = SAElementUtilities.CreateFloatField(TextureBufferNode.mipMapBias, "Mipmap Bias", evt =>
            {
        ...
        private VisualElement Memory()
        {
            ...
            var enableRandomWrite = SAElementUtilities.CreateToggle(TextureBufferNode.enableRandomWrite,
                "Random Write",
                evt =>
                {
            ...
            var memoryless = SAElementUtilities.CreateMaskField((int) TextureBufferNode.memoryless, "Memoryless",
                new List<string>()
                {
                    "Color", "Depth", "MSAA"
                }, evt =>
            ...
            var inFastMemory = SAElementUtilities.CreateToggle(TextureBufferNode.fastMemoryDesc.inFastMemory,
                "In Fast Memory",
                evt =>
                {
            ...
            var fastMemoryFlags = SAElementUtilities.CreateMaskField((int)TextureBufferNode.fastMemoryDesc.flags, "Flags",
                new List<string>()
                {
            ...
            var residencySlider = SAElementUtilities.CreateSlider(0f, 1f,
            ...
            var depthAccess = SAElementUtilities.CreateEnumField(TextureBufferNode.depthAccess, "Depth Access", evt =>
            {
        ...
        private VisualElement MSAA()
        {
            ...
            var msaa = SAElementUtilities.CreateEnumField(TextureBufferNode.msaaSamples, "MSAA Samples", evt =>
            {
                TextureBufferNode.msaaSamples = (MSAASamples)evt.newValue;
            });
            root.Add(msaa);

            var bindSample = SAElementUtilities.CreateToggle(TextureBufferNode.bindTextureMS, "Bind Multip-Sample",
                evt =>
                {
